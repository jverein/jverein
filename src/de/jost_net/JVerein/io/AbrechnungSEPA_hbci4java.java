...
//   Abrechnungsläufe die die Ausgabe DATEI nutzen, können somit die aktuellen SEPA-XML Schematas verwenden
//            
// -------------------------------------------------
// Aenderungshistorie:
...
public class AbrechnungSEPA_hbci4java
{
	...
	private void lastschrift_fillup(JVereinZahler zahler) throws Exception
	{
		...
		{
			// System.out.println("rcur: counter: " + counter_rcur + ", name : " +
			// zahler.getName() + ", wert : " + zahler.getBetrag());
			// Jetzt die Daten vom Zahler in das LastschriftRCUR Property schreiben
			counter_rcur++;
			counter_index = counter_rcur - 1;
			lastschriftRCUR.setProperty(
					SepaUtil.insertIndex("dst.bic", counter_index),
					StringUtils.trimToEmpty(zahler.getBic()));
			lastschriftRCUR.setProperty(
					SepaUtil.insertIndex("dst.iban", counter_index),
					StringUtils.trimToEmpty(zahler.getIban()));
			lastschriftRCUR.setProperty(
					SepaUtil.insertIndex("dst.name", counter_index),
					StringUtils.trimToEmpty(zahler.getName()));
			lastschriftRCUR.setProperty(
					SepaUtil.insertIndex("btg.value", counter_index), betrag);
			lastschriftRCUR.setProperty(
					SepaUtil.insertIndex("btg.curr", counter_index),
					HBCIProperties.CURRENCY_DEFAULT_DE);
			lastschriftRCUR.setProperty(SepaUtil.insertIndex("usage", counter_index),
					StringUtils.trimToEmpty(zahler.getVerwendungszweck()));
			lastschriftRCUR.setProperty(
					SepaUtil.insertIndex("endtoendid", counter_index), "NOTPROVIDED");
			lastschriftRCUR.setProperty(
					SepaUtil.insertIndex("creditorid", counter_index), creditorid);
			lastschriftRCUR.setProperty(
					SepaUtil.insertIndex("mandateid", counter_index),
					StringUtils.trimToEmpty(zahler.getMandatid()));
			lastschriftRCUR.setProperty(
					SepaUtil.insertIndex("manddateofsig", counter_index),
					ISO_DATE.format(zahler.getMandatdatum()));
			lastschriftRCUR.setProperty(
					SepaUtil.insertIndex("purposecode", counter_index), "OHTR");
		}
		else
		{
			// System.out.println("frst: counter: " + counter_frst + ", name : " +
			// zahler.getName() + ", wert : " + zahler.getBetrag());
			// Jetzt die Daten vom Zahler in das LastschriftFRST Property schreiben
			counter_frst++;
			counter_index = counter_frst - 1;
			lastschriftFRST.setProperty(
					SepaUtil.insertIndex("dst.bic", counter_index),
					StringUtils.trimToEmpty(zahler.getBic()));
			lastschriftFRST.setProperty(
					SepaUtil.insertIndex("dst.iban", counter_index),
			...
			lastschriftFRST.setProperty(
					SepaUtil.insertIndex("btg.curr", counter_index),
					HBCIProperties.CURRENCY_DEFAULT_DE);
			lastschriftFRST.setProperty(SepaUtil.insertIndex("usage", counter_index),
					StringUtils.trimToEmpty(zahler.getVerwendungszweck()));
			lastschriftFRST.setProperty(
					SepaUtil.insertIndex("endtoendid", counter_index), "NOTPROVIDED");
			lastschriftFRST.setProperty(
	...
	// Hauptfunktion für die SEPA-Lastschrift mit hbci4java
	public AbrechnungSEPA_hbci4java(AbrechnungSEPAParam param,
			...
			throws Exception
	{
		...
		// 20220803: sbuer: Relikt aus obantoo und bleibt erstmal als DUMMY erhalten
		// und wird nicht mehr gefuellt
		JVereinBasislastschrift lastschrift = new JVereinBasislastschrift();

		// 20220803: sbuer: Anpassungen für hbci4java
		PainVersionDialog d = new PainVersionDialog(
		...
		// Initialdaten für die Lastschriften FRST und RCUR
		lastschriftFRST.setProperty("src.bic",
				StringUtils.trimToEmpty(Einstellungen.getEinstellung().getBic()));
		lastschriftFRST.setProperty("src.iban",
				StringUtils.trimToEmpty(Einstellungen.getEinstellung().getIban()));
		lastschriftFRST.setProperty("src.name",
				StringUtils.trimToEmpty(Einstellungen.getEinstellung().getName()));
		...
		lastschriftFRST.setProperty("targetdate",
				abrl.getStichtag() != null ? ISO_DATE.format(abrl.getStichtag())
						: SepaUtil.DATE_UNDEFINED);
		lastschriftFRST.setProperty("type", "CORE");
		lastschriftFRST.setProperty("batchbook", "");

		lastschriftRCUR.setProperty("src.bic",
				StringUtils.trimToEmpty(Einstellungen.getEinstellung().getBic()));
		lastschriftRCUR.setProperty("src.iban",
				StringUtils.trimToEmpty(Einstellungen.getEinstellung().getIban()));
		lastschriftRCUR.setProperty("src.name",
				StringUtils.trimToEmpty(Einstellungen.getEinstellung().getName()));
		...
		lastschriftRCUR.setProperty("targetdate",
				abrl.getStichtag() != null ? ISO_DATE.format(abrl.getStichtag())
						: SepaUtil.DATE_UNDEFINED);
		...
		// In diesen Methoden werden nun die Lastschriften mit der Methode
		// lastschrift_fillup gefuellt
		...
		monitor.log(counter_all + " abgerechnete Fälle, davon " + counter_frst
				+ " FRST, " + counter_rcur + " RCUR");
		// Lastschrift Datei RCUR erzeugen: Aber nur wenn auch welche gezaehlt
		// wurden
		if (counter_frst > 0)
		{
			...
			Logger.info(
					"Genieren der SEPA-XML FRST erfolgreich : " + param.sepafileFRST);
			monitor.log(
					"Genieren der SEPA-XML FRST erfolgreich : " + param.sepafileFRST);
		}
		...
		if (counter_rcur > 0)
		{
			...
			Logger.info(
					"Genieren der SEPA-XML RCUR erfolgreich : " + param.sepafileRCUR);
			monitor.log(
					"Genieren der SEPA-XML RCUR erfolgreich : " + param.sepafileRCUR);
		}
			...
			Abrechnungslauf abrl, Konto konto) throws Exception
	{
		if (param.abbuchungsmodus != Abrechnungsmodi.KEINBEITRAG)
		{
			...
			list.addFilter("(eintritt <= ? or eintritt is null) ", new Object[]
			{
					new java.sql.Date(param.stichtag.getTime())
			});
			// Das Mitglied darf noch nicht ausgetreten sein
			list.addFilter("(austritt is null or austritt > ?)", new Object[]
			{
					new java.sql.Date(param.stichtag.getTime())
			});
			...
			{
				list.addFilter("eingabedatum >= ?", new Object[]
				{
					...
					.getBeitragsmodel() == Beitragsmodel.MONATLICH12631)
			{
				if (param.abbuchungsmodus == Abrechnungsmodi.HAVIMO)
				{
					list.addFilter(
							...
							new Object[]
							{
									new Integer(Zahlungsrhythmus.HALBJAEHRLICH),
				...
				if (param.abbuchungsmodus == Abrechnungsmodi.JAVIMO)
				{
					list.addFilter(
							...
							new Object[]
							{
									new Integer(Zahlungsrhythmus.JAEHRLICH),
									new Integer(Zahlungsrhythmus.VIERTELJAEHRLICH),
									new Integer(Zahlungsrhythmus.MONATLICH)
							});
				}
			...
			while (list.hasNext())
			{
				...
				// Da alles in z ueberfuehrt wird, muß jetzt hier geprueft werden ob das
				// lastschrift property gefuellt werden kann
				if (z != null)
				{
...
// 20220813: sbuer: Diese Klasse ist durch die Kopie der Klasse AbrechnungSEPA
// entstanden.
// Damit die Klasse nicht doppelt vorhanden ist, wurde sie um den Namen DUMMY
// ergänzt
