...
public class PersonalbogenAction implements Action
{
  ...
  private void generierePersonalbogen(Mitglied[] m) throws IOException
  {
    ...
    fd.setFileName(new Dateiname(JVereinPlugin.getI18n().tr("personalbogen"),
      ...
      public void run(ProgressMonitor monitor) throws ApplicationException
      {
        try
        {
          Reporter rpt = new Reporter(new FileOutputStream(file), "",
              JVereinPlugin.getI18n().tr("Personalbogen"), mitglied.length);
          ...
          for (Mitglied m : mitglied)
          {
            ...
            rpt.add(
                JVereinPlugin.getI18n().tr("Personalbogen") + " "
                    + m.getVornameName(), 14);
      ...
      throws DocumentException, MalformedURLException, IOException
  {
    ...
    if (it.size() > 0)
    {
      ...
      {
        rpt.addColumn(JVereinPlugin.getI18n().tr("Foto"), Element.ALIGN_LEFT);
        rpt.addColumn(foto.getFoto(), 100, 100, Element.ALIGN_RIGHT);
      }
    }
    if (Einstellungen.getEinstellung().getExterneMitgliedsnummer())
    {
      rpt.addColumn(JVereinPlugin.getI18n().tr("Ext. Mitgliedsnummer"),
          Element.ALIGN_LEFT);
      rpt.addColumn(
    ...
    {
      rpt.addColumn(JVereinPlugin.getI18n().tr("Mitgliedsnummer"),
          Element.ALIGN_LEFT);
      rpt.addColumn(m.getID(), Element.ALIGN_LEFT);
    }
    rpt.addColumn(JVereinPlugin.getI18n().tr("Name, Vorname"),
        Element.ALIGN_LEFT);
    rpt.addColumn(m.getNameVorname(), Element.ALIGN_LEFT);
    rpt.addColumn(JVereinPlugin.getI18n().tr("Anschrift"), Element.ALIGN_LEFT);
    rpt.addColumn(m.getAnschrift(), Element.ALIGN_LEFT);
    rpt.addColumn(JVereinPlugin.getI18n().tr("Geburtsdatum"),
        Element.ALIGN_LEFT);
    rpt.addColumn(m.getGeburtsdatum(), Element.ALIGN_LEFT);
    if (m.getSterbetag() != null)
    {
      rpt.addColumn(JVereinPlugin.getI18n().tr("Sterbetag"), Element.ALIGN_LEFT);
      rpt.addColumn(m.getSterbetag(), Element.ALIGN_LEFT);
    }
    rpt.addColumn(JVereinPlugin.getI18n().tr("Geschlecht"), Element.ALIGN_LEFT);
    rpt.addColumn(m.getGeschlecht(), Element.ALIGN_LEFT);
    rpt.addColumn(JVereinPlugin.getI18n().tr("Kommunikation"),
        Element.ALIGN_LEFT);
    String kommunikation = "";
    if (m.getTelefonprivat().length() != 0)
    {
      kommunikation += JVereinPlugin.getI18n().tr("privat:") + " "
          + m.getTelefonprivat();
    }
    if (m.getTelefondienstlich().length() != 0)
    {
      ...
      kommunikation += JVereinPlugin.getI18n().tr("dienstlich:") + " "
    ...
    if (m.getHandy().length() != 0)
    {
      ...
      kommunikation += JVereinPlugin.getI18n().tr("Handy:") + " "
          + m.getHandy();
    }
    if (m.getEmail().length() != 0)
    {
      ...
      kommunikation += JVereinPlugin.getI18n().tr("Email:") + " "
          + m.getEmail();
    }
    rpt.addColumn(kommunikation, Element.ALIGN_LEFT);
    if (m.getAdresstyp().getID().equals("1"))
    {
      rpt.addColumn(JVereinPlugin.getI18n().tr("Eintritt"), Element.ALIGN_LEFT);
      rpt.addColumn(m.getEintritt(), Element.ALIGN_LEFT);
      rpt.addColumn(JVereinPlugin.getI18n().tr("Beitragsgruppe"),
          Element.ALIGN_LEFT);
      rpt.addColumn(
      ...
      {
        rpt.addColumn(JVereinPlugin.getI18n().tr("individueller Beitrag"),
            Element.ALIGN_LEFT);
    ...
    rpt.addColumn(JVereinPlugin.getI18n().tr("Datum Erstspeicherung"),
        Element.ALIGN_LEFT);
    rpt.addColumn(m.getEingabedatum(), Element.ALIGN_LEFT);
    rpt.addColumn(JVereinPlugin.getI18n().tr("Datum letzte Änderung"),
        Element.ALIGN_LEFT);
      ...
      throws RemoteException, DocumentException
  {
    ...
    if (it.size() > 0)
    {
      ...
      while (it.hasNext())
      {
        ...
        rpt.addColumn(JVereinPlugin.getI18n().tr("Soll"), Element.ALIGN_LEFT);
